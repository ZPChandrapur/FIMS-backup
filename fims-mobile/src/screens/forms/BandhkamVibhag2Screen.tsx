...
export default function BandhkamVibhag2Screen() {
  ...
  const handleSaveAsDraft = async () => {
    try {
      setLoading(true);
      await createInspection({ category_id: categoryId, inspector_id: user?.id, filled_by_name: formData.contractor || user?.email || '', status: 'draft', location_name: location?.address || 'Unknown Location', location_latitude: location?.latitude, location_longitude: location?.longitude, location_address: location?.address || null });
